...
@DisplayName("SpecificDateExchangeRatesDTOMapper Tests")
public final class SpecificDateExchangeRatesDTOMapperTests {

    @Test
    @DisplayName("SpecificDateExchangeRatesDTOMapper::map correctly maps valid DTO")
    void maps() {
        ...
        SpecificDateExchangeRatesDTO dataObject = new SpecificDateExchangeRatesDTO(
                unitedStatesDollar.code(),
                date,
                Map.of(swissFranc.code(), rateValue)
        );

        Set<ExchangeRate> expected = Set.of(new ExchangeRate(unitedStatesDollar, swissFranc, rateValue, date));
        Set<ExchangeRate> actual = SpecificDateExchangeRatesDTOMapper.INSTANCE.map(dataObject);
    ...
    void throwsOnUnidentifiedCurrency() {
        ...
        Assertions.assertThrows(DTOMappingException.class, () -> SpecificDateExchangeRatesDTOMapper.INSTANCE.map(dataObject));
    }
}
